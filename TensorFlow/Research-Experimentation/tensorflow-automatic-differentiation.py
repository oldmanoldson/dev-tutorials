...
def f(x, y):
    output = 1
    #Must use range(int(y)) instead of range(y) in Python 3 when using Tensorflow 1.10 and earlier. Can use range(y) in 1.11+
    for i in range(y):
...
#Gradient of the output with respect to intermediate values computed in the function
x = tf.ones((2, 2))
